...
class GZipKNNClassifier:
    ...
    def __init__(self, k: int = 5, compresslevel: int = 6, max_prompt_size: int = 50000):
        """
        ...
        Args:
            ...
            max_prompt_size: Maximum allowed prompt size in bytes to prevent resource exhaustion
        """
        self.k = k
        ...
        self._query_compression_cache: Dict[int, int] = {}  # Cache query compressions
    ...
    def _ncd(self, x: str, y: str, cx: Optional[int] = None) -> float:
        """
        ...
        Args:
            x: First string
            y: Second string
            cx: Optional pre-computed compression of x (optimization)
    ...
    def classify(self, query: str) -> ClassificationResult:
        ...
        Raises:
            ValueError: If query exceeds max_prompt_size
        """
        # Validate prompt size to prevent resource exhaustion
        query_bytes = query.encode('utf-8')
        if len(query_bytes) > self.max_prompt_size:
            raise ValueError(
                f"Query size ({len(query_bytes)} bytes) exceeds maximum allowed "
                f"({self.max_prompt_size} bytes). This limit protects against resource exhaustion."
            )
        ...
        # Cache query compression to avoid recomputing
        query_hash = hash(query)
        if query_hash not in self._query_compression_cache:
            self._query_compression_cache[query_hash] = len(
                gzip.compress(query_bytes, self.compresslevel)
            )
        
        Cx = self._query_compression_cache[query_hash]

        distances = []
        
        # ⚠️ RESOURCE EXHAUSTION WARNING
        # This loop performs repeated synchronous gzip compression for every training example.
        # For large prompts or large training sets, this can exhaust routing worker resources.
        # If resource exhaustion becomes a bottleneck, implement one of these mitigations:
        #   1. Early Termination: Stop once k neighbors from same category are found
        #   2. Adaptive k: Reduce k dynamically for large inputs (e.g., k=1 for >5KB queries)
        #   3. Sampling: Sample M examples instead of comparing all N examples
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "query_cache_size": len(self._query_compression_cache),
